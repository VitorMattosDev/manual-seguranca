...
- **Variável de `\foreach` no TikZ nunca pode ter nome de macro de acento do LaTeX.** `\c`, `\d`, `\t`, `\i`, `\u`, `\v`, `\r`, `\b`, `\k`, `\l`, `\o`, `\a` são comandos de acentuação — redefini-los num `\foreach` e usar texto acentuado dentro do laço causa recursão infinita e o build morre com `TeX capacity exceeded [input stack size=10000]`, apontando para a linha errada. Usar sempre nomes de duas letras ou mais (`\idx`, `\tit`, `\des`, `\ctr`, `\cor`).
